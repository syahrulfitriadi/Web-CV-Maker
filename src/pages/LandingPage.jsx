...
export default function LandingPage() {
  ...
  return (
    <div style={{ paddingTop: 64 }}>
      {/* Hero */}
      <section style={{ minHeight: isMobile ? 'auto' : 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden', padding: isMobile ? '40px 0 20px' : undefined }}>
        {/* BG decorations */}
        <div style={{ position: 'absolute', top: -120, right: -120, width: isMobile ? 250 : 400, height: isMobile ? 250 : 400, background: 'rgba(14,165,233,0.08)', borderRadius: '50%', filter: 'blur(80px)' }} />
        <div style={{ position: 'absolute', bottom: -120, left: -120, width: isMobile ? 250 : 400, height: isMobile ? 250 : 400, background: 'rgba(14,165,233,0.06)', borderRadius: '50%', filter: 'blur(80px)' }} />

        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '20px 20px' : '40px 24px', width: '100%' }}>
          <div style={{ display: isMobile ? 'flex' : 'grid', flexDirection: 'column', gridTemplateColumns: isMobile ? undefined : '1fr 1fr', gap: isMobile ? 32 : 60, alignItems: 'center' }}>
            {/* Left Text */}
            <motion.div variants={containerVariants} initial="hidden" animate="visible">
              <motion.div variants={itemVariants}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 40, background: '#eff6ff', border: '1px solid #bfdbfe', color: '#0369a1', fontSize: isMobile ? 13 : 14, fontWeight: 500, marginBottom: isMobile ? 16 : 24 }}
              >
              ...
              <motion.h1 variants={itemVariants}
                style={{ fontSize: isMobile ? 32 : 48, fontWeight: 800, color: '#0f172a', lineHeight: 1.15, marginBottom: isMobile ? 14 : 20 }}
              >
                Buat CV{' '}
              ...
              <motion.p variants={itemVariants}
                style={{ fontSize: isMobile ? 15 : 17, color: '#64748b', lineHeight: 1.7, marginBottom: isMobile ? 24 : 32, maxWidth: 480 }}
              >
              ...
              <motion.div variants={itemVariants}>
                <motion.button
                  ...
                  style={{
                    display: 'inline-flex', alignItems: 'center', gap: 10, padding: isMobile ? '14px 28px' : '16px 36px',
                    borderRadius: 16, background: '#0ea5e9', color: 'white', fontWeight: 600,
                    fontSize: isMobile ? 16 : 17, border: 'none', cursor: 'pointer',
              ...
              <motion.div variants={itemVariants}
                style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 16 : 24, marginTop: isMobile ? 20 : 28, fontSize: 13, color: '#94a3b8', flexWrap: 'wrap' }}
              >
            ...
            {/* Right — CV Mockup (hidden on mobile) */}
            {!isMobile && (
              <motion.div
      ...
      <section style={{ padding: isMobile ? '48px 0' : '80px 0', background: 'white' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          ...
          >
            <h2 style={{ fontSize: isMobile ? 24 : 34, fontWeight: 800, color: '#0f172a', marginBottom: 12 }}>
              Kenapa Memilih <span className="gradient-text">CV Builder</span>?
            </h2>
            <p style={{ color: '#64748b', fontSize: isMobile ? 15 : 17 }}>Semua yang Anda butuhkan untuk membuat CV profesional ada di sini</p>
          </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isMobile ? 12 : 20 }}>
            {features.map((f, i) => (
              <motion.div key={f.title}
                ...
                style={{
                  padding: isMobile ? 18 : 28, borderRadius: isMobile ? 16 : 20, background: '#f8fafc',
                  border: '1px solid #e2e8f0', cursor: 'pointer',
                  transition: 'all 0.3s',
                }}
              >
                <div style={{
                  width: isMobile ? 40 : 48, height: isMobile ? 40 : 48, borderRadius: isMobile ? 12 : 14, background: f.gradient,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: isMobile ? 12 : 16,
                }}>
                  <f.icon style={{ width: isMobile ? 20 : 24, height: isMobile ? 20 : 24, color: 'white' }} />
                </div>
                <h3 style={{ fontWeight: 700, color: '#1e293b', marginBottom: 8, fontSize: isMobile ? 14 : 16 }}>{f.title}</h3>
                <p style={{ fontSize: isMobile ? 12 : 14, color: '#64748b', lineHeight: 1.6 }}>{f.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: isMobile ? '48px 0' : '80px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            style={{ textAlign: 'center', marginBottom: isMobile ? 32 : 56 }}
          >
            <h2 style={{ fontSize: isMobile ? 24 : 34, fontWeight: 800, color: '#0f172a', marginBottom: 8 }}>Cara Membuat CV</h2>
            <p style={{ color: '#64748b', fontSize: isMobile ? 15 : 17 }}>3 langkah mudah</p>
          </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: isMobile ? 28 : 40, textAlign: 'center' }}>
            {[
              ...
              <motion.div key={item.n} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}>
                <div style={{ fontSize: isMobile ? 48 : 72, fontWeight: 900, color: '#dbeafe', lineHeight: 1 }}>{item.n}</div>
                <h3 style={{ fontSize: isMobile ? 18 : 20, fontWeight: 700, color: '#1e293b', marginTop: -12, marginBottom: 8 }}>{item.t}</h3>
                <p style={{ color: '#64748b', fontSize: isMobile ? 14 : 15 }}>{item.d}</p>
              </motion.div>
            ))}
          </div>

          <div style={{ textAlign: 'center', marginTop: isMobile ? 36 : 56 }}>
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
              onClick={() => setCurrentStep(1)}
              style={{
                display: 'inline-flex', alignItems: 'center', gap: 10, padding: isMobile ? '14px 28px' : '16px 36px',
                borderRadius: 16, color: 'white', fontWeight: 600, fontSize: isMobile ? 16 : 17, border: 'none',
      ...
      <footer style={{ padding: '24px 0', borderTop: '1px solid #e2e8f0', textAlign: 'center', fontSize: isMobile ? 12 : 14, color: '#94a3b8' }}>
